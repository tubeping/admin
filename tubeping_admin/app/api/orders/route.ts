...
export async function GET(request: NextRequest) {
  ...
  while (allData.length < limit) {
    let query = sb
      .from("orders")
      .select(
        "*, stores:store_id(name, mall_id), suppliers:supplier_id(name, email, po_config), purchase_orders:purchase_order_id(id, po_number, status, sent_at, viewed_at, completed_at)"
      )
  ...
  if (orders.length > 0) {
    ...
    // (출고지는 상품 마스터 출고창고 값만 사용 — 주문 단위 수동 override는 더 이상 쓰지 않음)
    ...
    for (const o of orders as any[]) {
      ...
      // 출고지 결정: 상품 마스터 출고창고 값만 사용 (미지정 → 자체배송)
